...
def remover_sublinhados_duplos(itens):
    ...
    for item in itens:
        ...
        nome_modificado = nome_original.replace('--', '-')  # Substitui sublinhados duplos por um único sublinhado
        while '--' in nome_modificado:  # Continua substituindo enquanto houver sublinhados duplos
            nome_modificado = nome_modificado.replace('--', '-')
        ...
        if nome_original != nome_modificado:
            ...
            print(f"{nome_original.replace('--', Back.YELLOW + '--' + Back.RESET)} → {nome_modificado}")
        else:
